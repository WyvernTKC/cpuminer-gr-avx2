...
int groestl512_4way_init(groestl512_4way_context *ctx,
                         uint64_t hashlen __attribute__((unused))) {
...
int groestl512_4way_update_close(groestl512_4way_context *ctx, void *output,
  ...
  const size_t len = (int)databitlen / 128;
  const size_t hashlen_m128i = 64 / 16; // bytes to __m128i
...
int groestl512_4way_full(groestl512_4way_context *ctx, void *output,
  ...
  const size_t len = (int)datalen >> 4;
  const size_t hashlen_m128i = 64 >> 4; // bytes to __m128i
  const size_t hash_offset = SIZE512 - hashlen_m128i;
  uint64_t blocks = len / SIZE512;
  __m512i *in = (__m512i *)input;
  size_t i;
...
int groestl512_2way_init(groestl512_2way_context *ctx,
                         uint64_t hashlen __attribute__((unused))) {
  if (ctx->chaining == NULL || ctx->buffer == NULL)
    return 1;
