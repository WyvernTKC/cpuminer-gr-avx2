...
int x16r_hash_generic(void *output, const void *input, int thrid) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    case CUBEHASH:
      if (i == 0)
        cubehashUpdateDigest(&ctx.cube, (byte *)hash, (const byte *)in + 64,
                             16);
      else
    ...
    case SHAVITE:
      ...
      break;
    case SIMD:
      simd_full(&ctx.simd, (unsigned char *)hash, (const unsigned char *)in,
                size << 3);
      break;
    case ECHO:
#if defined(__AES__)
      echo_full(&ctx.echo, (unsigned char *)hash, 512,
...
int scanhash_x16r(struct work *work, uint32_t max_nonce, uint64_t *hashes_done,
