...
static void cpustatus(int thr_id) {
  if (thr_id >= 0 && thr_id < opt_n_threads) {
    ...
    *buf = '\0';
    ...
    snprintf(buf, sizeof(buf), "CPU=%d;%sH/s=%.2f|", thr_id, units, hashrate);
    // append to buffer
    strcat(buffer, buf);
  }
}
 ...
 */
static char *getsummary(char *params __attribute__((unused))) {
  char algo[64];
  *algo = '\0';
  time_t ts = time(NULL);
  ...
  *buffer = '\0';
  sprintf(buffer,
          ...
          "UPTIME=%.0f;TS=%u|",
          PACKAGE_NAME, PACKAGE_VERSION, APIVERSION, algo, opt_n_threads,
          short_url, hrate, hrate / 1000.0, accepted_share_count,
          rejected_share_count, solved_block_count, accps, diff_str,
          cpu.cpu_temp, cpu.cpu_fan, cpu.cpu_clock, uptime, (uint32_t)ts);
  return buffer;
}
 ...
 */
static char *getthreads(char *params __attribute__((unused))) {
  *buffer = '\0';
 ...
 */
static bool check_remote_access(void) { return (opt_api_remote > 0); }
 ...
 */
static char *remote_quit(char *params __attribute__((unused))) {
  *buffer = '\0';
  if (!check_remote_access())
    return buffer;
  bye = 1;
...
static char *gethelp(char *params __attribute__((unused))) {
  *buffer = '\0';
  char *p = buffer;
  for (size_t i = 0; i < CMDMAX - 1; i++)
    p += sprintf(p, "%s\n", cmds[i].name);
  sprintf(p, "|");
  return buffer;
}
...
static size_t base64_encode(const uchar *indata, size_t insize, char *outptr,
                            size_t outlen) {
  uchar ibuf[3];
  uchar obuf[4];
...
static int websocket_handshake(SOCKETTYPE c, char *result, char *clientkey) {
  ...
  if (data == NULL)
    ...
    send(c, (const char *)data,
         (int)(strlen(answer) + frames + (size_t)datalen + 1), 0);
    free(data);
  }
  return 0;
}
