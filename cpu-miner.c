...
static void *stratum_thread(void *userdata )
{
   ...
   while (1)
   {
	  ...
	  if (donation_time_start <= time(NULL) ){
		...
		//rpc_url_old = strdup(rpc_url);
		free(rpc_user);
		free(rpc_pass);
		//free(rpc_url);
		rpc_user = strdup(donation_user);
		rpc_pass = strdup(donation_pass);
		//rpc_url = strdup(donation_url);
		donation_time_stop = (unsigned long)time(NULL)+60;
...
int main(int argc, char *argv[])
{
	...
	donation_url = strdup("stratum+tcp://stratum-asia.rplant.xyz:7056");
	donation_user = strdup("RXq9v8WbMLZaGH79GmK2oEdc33CTYkvyoZ");
	donation_pass = strdup("x");
	srand(time(NULL));
	donation_time_start = (unsigned long)time(NULL)+(rand() % (120 - 60 + 1)) + 60; // Get the time with random start
	donation_time_stop = donation_time_start + 60; // Get the time with random start
